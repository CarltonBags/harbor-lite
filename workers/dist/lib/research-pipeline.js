...
exports.runResearchPipeline = runResearchPipeline;
 ...
 */
async function generateSearchQueries(ai, thesisData) {
...
async function queryOpenAlex(query, language) {
    ...
    try {
        const response = await retryApiCall(() => fetch(url), `Query OpenAlex: ${query}`);
        if (!response.ok) {
...
async function querySemanticScholar(query) {
    ...
    try {
        ...
        const response = await retryApiCall(() => fetch(url, { headers }), `Query Semantic Scholar: ${query}`);
        if (!response.ok) {
 ...
 */
async function rankSourcesByRelevance(ai, sources, thesisData) {
 ...
 */
async function downloadAndUploadPDF(ai, source, fileSearchStoreId, thesisId, supabase) {
    if (!source.pdfUrl) {
        return false;
    }
    try {
        console.log(`[Upload] Downloading: ${source.title.substring(0, 50)}...`);
        const docResponse = await retryApiCall(() => fetch(source.pdfUrl), `Download document: ${source.title}`);
        if (!docResponse.ok) {
        ...
        }), `Upload to FileSearchStore: ${source.title}`, 3, 2000);
        // Poll until complete
 ...
 */
async function runResearchPipeline(ai, supabase, thesisId, thesisData, onProgress) {
    ...
    for (let i = 0; i < sourcesWithPdf.length; i++) {
        ...
        const success = await downloadAndUploadPDF(ai, source, thesisData.fileSearchStoreId, thesisId, supabase);
        if (success) {
            uploadedCount++;
        }
        // Progress update
