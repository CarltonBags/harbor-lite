...
const jobQueue = [];
/**
 * Convert thesis content to clean Markdown format
 * Ensures proper heading structure and formatting for exports
 */
function convertToCleanMarkdown(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Strip bold from long paragraphs (>150 chars) - AI sometimes bolds entire paragraphs
        ...
        // Check existing headings and normalize them
...
function detectChapters(content, outlineChapters) {
    if (!content || !outlineChapters.length)
        return [];
    const found = [];
    outlineChapters.forEach((chapter) => {
        ...
        const patterns = [];
        if (number && title) {
            patterns.push(new RegExp(`(?:^|\\n)#+\\s*${escapeRegex(number)}\\s+.*${escapeRegex(title)}`, 'i'));
        }
        if (number) {
...
function getMissingChapters(content, outlineChapters) {
...
function getRecentExcerpt(content, maxChars = 8000) {
    if (!content)
        return '';
...
function buildOutlineSummary(outlineChapters) {
    if (!outlineChapters.length)
        return '';
    return outlineChapters
        .map((chapter) => `- ${formatChapterLabel(chapter)}`)
...
async function retryApiCall(fn, operationName, maxRetries = 3, baseDelay = 1000) {
    let lastError;
 ...
 */
async function generateSearchQueries(thesisData) {
 ...
 */
async function queryOpenAlex(query, language) {
    ...
    try {
        ...
        const response = await retryApiCall(() => (0, node_fetch_1.default)(url), `Query OpenAlex: ${query}`);
        const duration = Date.now() - startTime;
        if (!response.ok) {
            console.error(`[OpenAlex] ERROR: ${response.status} ${response.statusText} for query: "${query}"`);
            return [];
        }
        const data = await retryApiCall(() => response.json(), `Parse OpenAlex response: ${query}`);
        ...
        const withPdf = sources.filter((s) => s.pdfUrl).length;
        const withDoi = sources.filter((s) => s.doi).length;
...
async function querySemanticScholar(query) {
    ...
    try {
        ...
        const startTime = Date.now();
        const response = await retryApiCall(() => (0, node_fetch_1.default)(url, { headers }), `Query Semantic Scholar: ${query}`);
        const duration = Date.now() - startTime;
        if (!response.ok) {
        ...
        const data = await retryApiCall(() => response.json(), `Parse Semantic Scholar response: ${query}`);
        ...
        const withPdf = sorted.filter((s) => s.pdfUrl).length;
        const withDoi = sorted.filter((s) => s.doi).length;
...
async function queryUnpaywall(doi) {
    ...
    try {
        ...
        const response = await retryApiCall(() => (0, node_fetch_1.default)(url, {
            headers: {
        ...
        const data = await retryApiCall(() => response.json(), `Parse Unpaywall response: ${doi}`);
 ...
 */
async function deduplicateAndEnrichSources(sources) {
    ...
    const enrichedSources = await Promise.all(deduplicated.map(async (source) => {
 ...
 */
async function rankSourcesByRelevance(sources, thesisData) {
...
Die Indizes entsprechen der Reihenfolge der Quellen im Input (0 bis ${batch.length - 1}).`;
        try {
            ...
            const batchRankings = JSON.parse(jsonMatch[0]);
    ...
    const mediumRelevance = sorted.filter((s) => (s.relevanceScore || 0) >= 40 && (s.relevanceScore || 0) < 70).length;
    const lowRelevance = sorted.filter((s) => (s.relevanceScore || 0) < 40).length;
 ...
 */
function selectTopSourcesWithChapterGuarantee(rankedSources, maxSources = 50, minPerChapter = 2) {
    ...
    const usedSources = new Set(); // Track by DOI or title to avoid duplicates
...
async function extractPageNumbers(pdfBuffer) {
    ...
    try {
        ...
        const uploadedFile = await retryApiCall(() => ai.files.upload({
            file: blob,
        }), 'Upload PDF to Gemini Files API');
 ...
 */
async function downloadAndUploadPDF(source, fileSearchStoreId, thesisId) {
    if (!source.pdfUrl) {
    ...
    try {
        ...
        const docResponse = await retryApiCall(() => (0, node_fetch_1.default)(source.pdfUrl), `Download document: ${source.title}`);
        ...
        let operation;
        try {
            operation = await retryApiCall(() => ai.fileSearchStores.uploadToFileSearchStore({
                file: fileSource,
            ...
            }), `Upload to FileSearchStore: ${source.title}`, 3, // 3 retries
        ...
        while (!operation.done) {
            ...
            pollCount++;
            const updatedOperation = await retryApiCall(() => ai.operations.get({ operation }), `Poll upload operation: ${source.title}`);
        ...
        // Update database
        const { data: thesis } = await retryApiCall(async () => {
        ...
        const existingSources = thesis?.uploaded_sources || [];
        const newSource = {
            doi: source.doi,
            title: source.title,
...
async function generateEmbedding(text) {
    ...
    try {
        ...
        const response = await retryApiCall(() => (0, node_fetch_1.default)('https://api.openai.com/v1/embeddings', {
            method: 'POST',
            headers: {
        ...
        }), `Generate embedding (OpenAI ${OPENAI_EMBEDDING_MODEL})`, 2, 1000);
        if (!response.ok) {
 ...
 */
async function chunkAndStoreThesis(thesisId, latexContent, outline) {
    ...
    for (let chapterIdx = 0; chapterIdx < chapters.length; chapterIdx++) {
        ...
        const chapterParagraphs = textContent.slice(paragraphIndex, Math.min(paragraphIndex + paragraphsPerChapter, textContent.length));
 ...
 */
async function sendCompletionEmail(thesisId, thesisTitle) {
 ...
 */
async function generateThesisPlan(thesisData, sources) {
    ...
    const sourcesList = sources.map((s, i) => `[${i + 1}] ${s.title} (${s.year || 'n.d.'}) - ${s.authors.join(', ')}\n   Abstract: ${s.abstract ? s.abstract.substring(0, 300) + '...' : 'No abstract'}`).join('\n\n');
    const outlineStr = thesisData.outline?.map((ch) => `${ch.number} ${ch.title}\n${ch.subchapters?.map((sub) => `  ${sub.number} ${sub.title}`).join('\n') || ''}`).join('\n');
    const prompt = `
...
function extractChapterPlan(thesisPlan, chapter, language) {
    if (!thesisPlan)
        return '';
...
function extractChapterWordTargets(thesisPlan, outlineChapters, totalWordTarget, language) {
    if (!outlineChapters.length)
        return [];
...
function formatSectionsSummary(chapter) {
...
async function extendThesisContent({ thesisData, thesisPlan, currentContent, expectedWordCount, outlineChapters, isGerman, }) {
    ...
    for (let pass = 1; pass <= maxPasses && wordCount < expectedWordCount; pass++) {
        ...
        const extensionTargetWords = Math.min(remainingWords, Math.min(4500, Math.max(roughTarget, Math.ceil(remainingWords / (maxPasses - pass + 1)))));
        ...
        const extensionResponse = await retryApiCall(() => ai.models.generateContent({
            model: 'gemini-2.5-pro',
        ...
        }), `Extend thesis content (pass ${pass})`, 1, 2000);
...
async function generateChapterContent({ thesisData, chapter, chapterTargetWords, thesisPlan, previousContent, isGerman, }) {
    ...
    let chapterContent = '';
    let attempts = 0;
    const buildChapterPrompt = (remainingWords) => {
...
${startInstruction}`;
    ...
    while (attempts < 3) {
        ...
        }), `Generate chapter ${chapterLabel} (attempt ${attempts})`, 1, 2000);
...
async function generateThesisContent(thesisData, rankedSources, thesisPlan = '') {
    ...
    const outlineChapters = (thesisData.outline || []).map((chapter, index) => ({
        number: (chapter?.number ?? `${index + 1}.`).toString().trim(),
        title: (chapter?.title ?? '').toString().trim(),
        sections: (chapter?.sections || chapter?.subchapters || []).map((section, sectionIndex) => ({
    ...
    let recommendedSourceCount;
    let sourceUsageGuidance;
    if (targetPages < 10) {
    ...
    if (useChapterGeneration) {
        ...
        const shouldSkipChapter = (chapter) => {
...
${thesisData.language === 'german' ? sourceUsageGuidance : sourceUsageGuidance.replace(/Sehr kurze Arbeit \((\d+) Seiten\): Verwende nur (\d+)-(\d+) hochwertige|Kurze Arbeit \((\d+) Seiten\): Verwende (\d+)-(\d+) sorgfältig|Mittlere Arbeit \((\d+) Seiten\): Verwende (\d+)-(\d+) relevante|Längere Arbeit \((\d+) Seiten\): Verwende (\d+)-(\d+) Quellen/g, (match) => {
            if (match.includes('Sehr kurze'))
                return `Very short thesis (${targetPages} pages): Use only ${recommendedSourceCount}-${recommendedSourceCount + 2} high-quality`;
            if (match.includes('Kurze'))
                return `Short thesis (${targetPages} pages): Use ${recommendedSourceCount}-${recommendedSourceCount + 3} carefully selected`;
            if (match.includes('Mittlere'))
                return `Medium thesis (${targetPages} pages): Use ${recommendedSourceCount}-${recommendedSourceCount + 5} relevant`;
            if (match.includes('Längere'))
                return `Longer thesis (${targetPages} pages): Use ${recommendedSourceCount}-${recommendedSourceCount + 10} sources`;
            return match;
        })}
...
DO NOT STOP until all requirements are met. The thesis must be COMPLETE.`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            }), `Generate thesis content (Gemini Pro + FileSearchStore) - Attempt ${attempt}`, 1, // Single retry per attempt (we're doing 3 attempts total)
            ...
            if (content && content.length > 100) {
                ...
                if (isSeriouslyIncomplete) {
                    ...
                    if (attempt < maxAttempts) {
                        const issues = [];
                        if (isTooShort)
                            issues.push(`too short (${wordCount}/${expectedWordCount} words)`);
                        if (isMissingBibliography)
                            issues.push('missing or empty bibliography');
                        if (hasPlaceholderBib)
                            issues.push('bibliography contains placeholder/example text');
                        if (!hasSufficientCitations)
                            issues.push(`insufficient citations (${citationCount}/${minCitations})`);
                        if (isMissingChapters && !wordCountMet)
                            issues.push(`missing chapters (${foundChapters.length}/${outlineChapters.length})`);
    ...
    const errorDetails = lastError instanceof Error
        ...
        : String(lastError);
    throw new Error(`Failed to generate thesis content after ${maxAttempts} attempts with FileSearchStore + Gemini Pro. ` +
 ...
 */
async function checkWithGPTZero(content) {
    if (!RAPIDAPI_KEY) {
    ...
    try {
        const response = await retryApiCall(() => (0, node_fetch_1.default)('https://zerogpt.p.rapidapi.com/api/v1/detectText', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
 ...
 */
async function rewriteFlaggedSentences(content, flaggedSentences, thesisData) {
...
["Rewritten sentence 1", "Rewritten sentence 2", ...]`;
        try {
            ...
            const rewrittenSentences = JSON.parse(jsonMatch[0]);
            ...
            batch.forEach((originalSentence, idx) => {
                if (rewrittenSentences[idx]) {
                    ...
                    rewrittenContent = rewrittenContent.replace(new RegExp(escapedOriginal, 'g'), rewrittenSentences[idx]);
                }
            });
 ...
 */
async function ensureHumanLikeContent(content, thesisData) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        currentContent = await rewriteFlaggedSentences(currentContent, result.gptGeneratedSentences, thesisData);
 ...
 */
async function humanizeThesisContent(content, thesisData) {
...
Your goal is to produce text that reads like it was written by a competent human academic author while preserving all informational content.`;
    try {
        const response = await retryApiCall(() => ai.models.generateContent({
            ...
            contents: `${prompt}\n\n---\n\n${content}`,
        }), 'Humanize thesis content', 3, // 3 retries
        2000 // 2 second delay
        );
 ...
 */
async function checkZeroGPT(content) {
    if (!RAPIDAPI_KEY) {
    ...
    try {
        ...
        const response = await retryApiCall(async () => {
            const fetchResponse = await (0, node_fetch_1.default)('https://zerogpt.p.rapidapi.com/api/v1/detectText', {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
                    'X-RapidAPI-Key': RAPIDAPI_KEY,
        ...
        }, 'Check text with ZeroGPT API', 3, // 3 retries
        2000 // 2 second delay
        );
...
function extractAndProcessFootnotes(content) {
    ...
    const sequentialFootnotes = {};
    const markerToSequential = new Map(); // marker index -> sequential number
 ...
 */
async function checkFileSearchStoreHasDocuments(fileSearchStoreId) {
    try {
 ...
 */
function convertUploadedSourcesToSources(uploadedSources) {
    return uploadedSources.map((uploaded) => {
        ...
        return {
            ...
            source: 'openalex', // Default to openalex (we don't track this in uploaded_sources)
 ...
 */
async function processThesisGeneration(thesisId, thesisData) {
    ...
    try {
        ...
        if (hasEnoughSources) {
            ...
            let chapterQueries = [];
            try {
                chapterQueries = await retryApiCall(() => generateSearchQueries(thesisData), 'Generate search queries', 3, 2000);
            ...
            for (const chapterQuery of chapterQueries) {
                const chapterNumber = chapterQuery.chapterNumber || chapterQuery.chapter || 'N/A';
                const chapterTitle = chapterQuery.chapterTitle || 'N/A';
            ...
            let deduplicated = [];
            try {
                deduplicated = await retryApiCall(() => deduplicateAndEnrichSources(allSources), 'Deduplicate and enrich sources', 2, // Fewer retries for this step
                1000);
            ...
            let ranked = [];
            try {
                ranked = await retryApiCall(() => rankSourcesByRelevance(deduplicated, thesisData), 'Rank sources by relevance', 2, // Fewer retries (this is already batched internally)
            ...
            const getSourceId = (source) => source.doi || source.title || '';
            ...
            const sourcesToProcess = [...topSources];
            let sourceIndex = 0;
                    ...
                    try {
                        ...
                        if (success) {
                            ...
                            candidates.sort((a, b) => {
                                ...
                                if (aSameChapter !== bSameChapter)
                                    return bSameChapter - aSameChapter;
        ...
        await retryApiCall(async () => {
            ...
            if (result.error)
                throw result.error;
            return result;
        }, `Update thesis status (error): ${thesisId}`).catch(err => {
...
app.get('/jobs/:thesisId', authenticate, async (req, res) => {
    const requestStart = Date.now();
...
const workerConnection = new ioredis_1.default(REDIS_URL, {
...
// Create the worker
const worker = new bullmq_1.Worker(THESIS_QUEUE_NAME, async (job) => {
